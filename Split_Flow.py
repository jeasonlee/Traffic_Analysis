...
if __name__ == '__main__':
    ...
    for key, value in tcp_flow_dict.items():
        try:
            print("ip.src:{0}, ip.dst:{1}, src_port:{2}, dst_port:{3} data:{4}".format(socket.inet_ntop(socket.AF_INET, key.src_ip),
                                                                                       socket.inet_ntop(socket.AF_INET, key.dst_ip),
                                                                                       key.src_port, key.dst_port,
                                                                                       len(value)))
        except:
            print("ip.src:{0}, ip.dst:{1}, src_port:{2}, dst_port:{3} data:{4}".format(socket.inet_ntop(socket.AF_INET6, key.src_ip),
